...
export default function PurchasesPage() {
  ...
  async function fetchPurchases(query?: string) {
    setLoading(true)
    setErr(null)
    const url =
      query && query.trim().length
        ? `/api/purchases?q=${encodeURIComponent(query.trim())}`
        : '/api/purchases'
    const res = await fetch(url, { cache: 'no-store' })
    const json = await res.json()
    if (!res.ok || !Array.isArray(json)) {
  ...
  const cols = rows.length
    ? [
        ...
        ...Object.keys(rows[0]).filter((k) => !preferred.includes(k)),
      ]
    : []
      ...
      <main className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-10">
        <div className="flex items-center gap-3">
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && fetchPurchases(q)}
            placeholder="Search purchases..."
            className="w-full rounded-md border border-black/10 bg-white px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-black dark:border-white/15 dark:bg-zinc-900 dark:focus:ring-white"
          />
          <button
            onClick={() => fetchPurchases(q)}
            className="rounded-md bg-black px-4 py-2 text-sm text-white transition-colors hover:bg-[#383838] dark:bg-white dark:text-black dark:hover:bg-[#ddd]"
          >
            {loading ? 'Searching…' : 'Search'}
          </button>
              ...
              <thead>
                <tr className="bg-zinc-100 dark:bg-zinc-900">
                  {cols.map((c) => (
                    <th
                      key={c}
                      className="border-b border-black/10 px-3 py-2 text-left font-semibold dark:border-white/15"
                    >
                      {c}
                    </th>
                  ))}
